...
rawImages=`find Images/Tiles/ -name "*.nef"`
outXMLFile="imageMetaData.xml"

# Each raw file follow this pattern:
#
#   LNNN_tag_HNN_WNN.nef
#
# where L is either T or W, the "part_" part optional, and the two subsequent numbers the
# height and width in millimeters. For instance: T41_H70_W50.nef.
...
for raw in $rawImages; do
    ...
    width=`echo "$basename" | sed -e 's/.*_W\([0-9]*\)\.nef$/\1/'`
